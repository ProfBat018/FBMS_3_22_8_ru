...
const Navbar: React.FC<NavbarProps> = ({ onLogin }) => {
  ...
  const { categories, loading, error, activeCategory, activeSubCategory } =
  ...
  const renderModalContent = () => {
    switch (modalContent) {
        case "login":
          return (
            <>
              <div className="text-xl font-bold black-950 mb-4">
                Log In
              </div>
  ...
  return (
    <nav className="flex relative items-center justify-between p-4 bg-black-950 text-white">
                  ...
                  >
                    ...
                    {activeCategory === category.name &&
                      category.subcategories.length > 0 && (
                        <div className="absolute left-full top-0 mt-2 w-48 bg-black-950 text-white shadow-lg">
                          {category.subcategories.map((subcat) => (
                            <div
                              key={subcat.name}
                              className="relative"
                              onMouseEnter={() =>
                                handleMouseEnterSubCategory(subcat.name)
                              }
                            >
                              <button
                                  ...
                                  <div className="absolute left-full top-0 mt-2 w-48 bg-black-950 text-white shadow-lg">
                                    {subcat.subcategories.map((subSubcat) => (
                                      <button
                                        key={subSubcat.name}
                                        onClick={() =>
                                          handleCategoryClick(subSubcat.id)
                                        }
                                        className="block px-4 py-2 hover:bg-gray-700"
                                      >
                                        {subSubcat.name}
      ...
      {isMenuOpen && (
        <div
          className={`absolute top-16 left-0 right-0 bg-black p-4 transition-transform duration-300 ${
        ...
        >
          ...
          <a className="block py-2 text-white hover:text-gray-300">
            Categories
          </a>
          <Link
            to="/about"
